...
@Component({
  selector: "shd-dual-listbox",
  templateUrl: "./dual-listbox.component.html",
  styleUrls: ["./dual-listbox.component.scss"],
})
export class DualListboxComponent implements DoCheck, OnChanges {
  ...
  static DEFAULT_FORMAT = {
    ...
    all: "All",
    none: "None",
    direction: DualListboxComponent.LTR,
    draggable: true,
    locale: undefined,
  };
  ...
  @Input() height = "100px";
  @Input() filter = true;
  @Input() format = DualListboxComponent.DEFAULT_FORMAT;
  @Input() sort = false;
  ...
  constructor(private differs: IterableDiffers) {
    this.available = new BasicList(DualListboxComponent.AVAILABLE_LIST_NAME);
    this.confirmed = new BasicList(DualListboxComponent.CONFIRMED_LIST_NAME);
  }
  ...
  ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
    ...
    if (changeRecord.format) {
      ...
      if (typeof this.format.direction === "undefined") {
        this.format.direction = DualListboxComponent.LTR;
      }
  ...
  direction() {
    return this.format.direction === DualListboxComponent.LTR;
  }
